...
router.delete('/supprimer/:id', async (req, res) => {
  const { id } = req.params;
  console.log(`📥 [DELETE /supprimer/:id] Demande de suppression pour la réservation id : ${id}`);

  try {
    const [result] = await db.query('DELETE FROM reservation WHERE id = ?', [id]);
    console.log(`[DELETE /supprimer/:id] Résultat SQL :`, result);
    if (result.affectedRows === 0) {
      console.log(`❌ [DELETE /supprimer/:id] Réservation non trouvée pour id : ${id}`);
      return res.status(404).json({ error: "Réservation non trouvée." });
    }
    console.log(`✅ [DELETE /supprimer/:id] Réservation supprimée avec succès, id : ${id}`);
    res.json({ message: "Réservation supprimée avec succès." });
  } catch (err) {
    console.error("❌ [DELETE /supprimer/:id] Erreur SQL :", err);
...
// 🔍 Récupérer la dernière réservation pour une voiture 
router.get('/last/:voiture', async (req, res) => {
...
// 🔍 Récupérer toutes les réservations d'une voiture
router.get('/Allreservations/byCar/:carId', async (req, res) => {
    const { carId } = req.params;
    console.log('📥 [GET /reservations/byCar/:carId] carId reçu :', carId);
    try {
        const [rows] = await db.query('SELECT * FROM reservation WHERE voiture = ?', [carId]);
        console.log('📤 [GET /reservations/byCar/:carId] Réservations récupérées :', rows);
        res.json(rows);
    } catch (err) {
        console.error('❌ [GET /reservations/byCar/:carId] Erreur SQL :', err);
        res.status(500).json({ error: "Erreur lors de la récupération des réservations." });
    }
});

// 🔍 Récupérer toutes les réservations des voitures d'un propriétaire où livraison > 0 et statut = 1
router.get('/livraisons/byOwnerLivree/:proprio', async (req, res) => {
    const { proprio } = req.params;
    console.log('📥 [GET /reservations/byOwnerLivree/:proprio] proprio reçu :', proprio);
    try {
        const sql = `
            SELECT r.* FROM reservation r
            JOIN car c ON r.voiture = c.id
            WHERE c.proprio = ? AND r.livraison > 0 AND r.statut = 1
        `;
        const [rows] = await db.query(sql, [proprio]);
        console.log('📤 [GET /reservations/byOwnerLivree/:proprio] Réservations récupérées :', rows);
        res.json(rows);
    } catch (err) {
        console.error('❌ [GET /reservations/byOwnerLivree/:proprio] Erreur SQL :', err);
        res.status(500).json({ error: "Erreur lors de la récupération des réservations." });
    }
});
